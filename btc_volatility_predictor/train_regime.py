"""Training script for High/Low Volatility Regime Classification"""
...
def create_regime_dataloaders(data_path, window_size=48, batch_size=32,
                               test_days=10, val_ratio=0.15, percentile=50):
    """Create dataloaders for regime classification."""
...
def main():
    ...
    # Data - use median (50th percentile) as threshold
    train_loader, val_loader, test_loader, metadata = create_regime_dataloaders(
        config.data_path,
    ...
    for epoch in range(config.epochs):
        ...
        if val_metrics['auc'] > best_val_auc:
            ...
            }, f"{config.checkpoint_dir}/best_regime_model.pt")
    ...
    checkpoint = torch.load(f"{config.checkpoint_dir}/best_regime_model.pt", weights_only=False)
    ...
    # Save results
    with open(f"{config.checkpoint_dir}/regime_results.json", 'w') as f:
        json.dump({
